...
/*
    ...
    즉, 상태를 바로바로 반영하지 않고, 여러 상태 업데이트를 배치해서 한번에 처리하려고 한다<div className="
...
const UseStateTest = () => {
    ...
    const onClickPlus = () =>{
        ...
        setNum(num+1);
        setNum(num+1);
        //결과적으로 num +1이 됨 -> 실시간으로 처리되는 것이 아니라 모아두었다가 한번에 처리하는 방식식        
    }

    const onClickMinus = () =>{
        setNum((prevNum) => {return prevNum -1});
        setNum((prevNum) => {console.log("이전상태 출력:",pre)
            return prevNum -1
